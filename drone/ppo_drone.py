...
def extract_config_with_defaults(config, section_name, default_values):
    """
    Extracts configuration values with defaults from a given section.
    
    Args:
        config (dict): Configuration dictionary.
        section_name (str): Name of the section to extract values from.
        default_values (dict): Default values for each key in the section.

    Returns:
        dict: Extracted values with defaults where necessary.
    """
...
def extract_attacker_settings(config):
    """
    Extracts settings specific to the attacker configuration.
    
    Args:
        config (dict): Configuration dictionary.
    
    Returns:
        dict: Attacker settings.
    """
...
def write_config(file_path, config_data):
    ...
    Args:
        file_path (str): Path to the file where the config will be written.
        config_data (dict): Configuration data to be written.
    """
...
if (docker):
    paths["px4_directory"] = "../PX4/PX4-Autopilot"
...
def train_defense_model(logdir, model_path, hyperparameters):
    ...
    if model_path:
        try:
            ...
            print("Error loading recovery model:", e)
            sys.exit(1)
    else:
...
def train_attacker_model(logdir, model_path, hyperparameters):
    # Save a checkpoint every save_freq steps
    attack_checkpoint_callback = CheckpointCallback(save_freq=1000, save_path=f'{logdir}/attacker_models/', name_prefix='attack_model')
    ...
    if (model_path):
        try:
            ...
            print("Error loading attacker model:", e)
            sys.exit(1)
    else:
...
def evaluate_defense_model(logdir, model_path, num_evaluation_episodes):
    ...
    for episode in range(num_evaluation_episodes + 1):
        obs = env.reset()
        done = False
...
if (args.evaluate > 0 and args.load_recovery != None):
...
if (args.evaluate > 0 and args.load_attacker != None):
