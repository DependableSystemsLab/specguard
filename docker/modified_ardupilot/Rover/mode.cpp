...
void Mode::calc_throttle(float target_speed, bool avoidance_enabled)
{
    ...
    const float MAX_THROTTLE_OUT = 25.0f;  // Adjust this value as needed
    throttle_out = constrain_float(throttle_out, -MAX_THROTTLE_OUT, MAX_THROTTLE_OUT);
...
void Mode::navigate_to_waypoint()
{
    ...
    if (override_flag) {
        calc_throttle(throttle_val, true);
    } else {
...
void Mode::calc_steering_from_turn_rate(float turn_rate)
{
    ...
    // Override steering controls if requested
    if (override_flag) {
        set_steering(steering_val); 
    } else {
