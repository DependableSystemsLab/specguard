...
#include "../../Rover/safesim_server.h"
...
void NavEKF3_core::calcOutputStates()
{
    ...
    // spoof gyro (yaw) values
    if (gyro_spoofing_flag) {
        // Create a quaternion representing the pitch rotation
        QuaternionF yawQuat;

        yawQuat.from_axis_angle(Vector3F(0, 0, 1), spoofed_yaw_offset);

        // Apply the yaw offset
        outputDataNew.quat *= yawQuat;
    ...
    if (runUpdates) {
        ...
        if (gps_spoofing_flag) {
            outputDataNew.position.x += spoofed_x_offset;
            outputDataNew.position.y += spoofed_y_offset;
        }
